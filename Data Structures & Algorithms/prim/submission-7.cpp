...
public:
    int minimumSpanningTree(vector<vector<int>>& edges, int n) {
        ...
        while (visit.size() < n && !minHeap.empty()) {
            /* We need both checks since visit.size() ensures that every 
            node in a connected graph is visited, and !minHeap.empty() 
            ensures that the while loop terminates for a disconnected graph, 
            i.e. after all the nodes in a single component have been visited
            */
        ...
        return visit.size() == n ? cost : -1; // -1 is for a disconnected graph (multiple components)
    }
};
