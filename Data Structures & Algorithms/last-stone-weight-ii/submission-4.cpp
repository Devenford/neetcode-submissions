...
class Solution {
public:
// The stones can be repeatedly smashed in pairs, where the resulting
// weight is the difference between the two stones. This is equivalent
// to dividing the stones into two groups and minimizing the difference
// between their total weights. Therefore, we use subset-sum DP to find
// the largest group sum <= sum/2, which gives the minimum possible difference.
